...
using StockMonitor.DayLineReader;
namespace ImageOfStock
{
    ...
    public partial class ChartGraph : ContainerControl
    {
         ...
         *技术部
         *杨航
         *边读边取，利用Reader把所有的条数体现出来，然后读数据的过程在操作的过程中进行
         * DisplayRectangle为整个图像范围
         */
        #region 构造函数
        /// <summary>
        /// 构造函数
        ...
        public ChartGraph()
        {
            ...
            this.KeyDown += new KeyEventHandler(ChartGraph_KeyDown);
        ...
        public string StockCode
        {
            set 
            {
                ...
                //初始化
                 stockCode = value;
                 KLineCount = (int)GetAllLineCount();
                }
            }
            get            
            {
        ...
        /// 是否为移动时需要加载并更改大小
        /// </summary>
        public bool IsOnMoveLoad = false;
        #endregion
        ...
        /// </summary>
        public DataTable dtAllMsg = new DataTable();
        ...
        public int LastVisibleRecord
        {
            ...
            set
            {
                ...
                if (this.dtAllMsg.Rows.Count > 0)
                {
                    ...
                    {
                        lastVisibleRecord = dtAllMsg.Rows.Count;
                    }
        ...
        /// 获取可视范围的值
        /// </summary>
        /// <param name="FirstVisble"></param>
        /// <param name="LastVisble"></param>
        public void LoadCursorValue(int FirstVisble,int LastVisble)
        {

            GetAreaLineToNow(FirstVisble, LastVisble, stockCode);
            KLineCount = (int)GetAllLineCount(stockCode);//所有数据
        ...
        public void CreateADay(string TXTpath, DayStockInfo dsi)
        {
            ...
            b_reader.Write(dsi.day.ToString("yyyyMMdd"));
            b_reader.Write(dsi.OpenPrice);
            b_reader.Write(dsi.HighestPrice);
            b_reader.Write(dsi.LowestPrice);
            b_reader.Write(dsi.ClosePrice);
            b_reader.Write(dsi.Amount);
            b_reader.Write(dsi.TransCount);
            b_reader.Write(dsi.Padding1);
            b_reader.Write(dsi.Padding2);
            b_reader.Write(dsi.Padding3);
            stream.Close();
        }
        /// <summary>
        /// 读取首日日期
        /// </summary>
        /// <param name="p_strFileName">路径</param>
        public DateTime ReadAFirstday(string p_strFileName)
        {
        ...
        public int GetDayPostion(DateTime DateStr, string p_strFileName)
        {
            ...
            int rowLength = 40;
            //日期
            int MyDate = int.Parse(DateStr.ToString("yyyyMMdd"));
            FileStream stream = new FileStream(p_strFileName, FileMode.Open, FileAccess.Read, FileShare.ReadWrite);
            BinaryReader b_reader = new BinaryReader(stream);
            //文件长度
        ...
        /// 数据读出事件
        /// </summary>
        public event StockInfoLayOut PicLineDrawValue;
        /// <summary>
        /// 数据读出结束事件
        /// </summary>
        public event StockInfoLayOutOver PicLineDrawValueOver;
        #endregion
        ...
        /// 获取两个时间内的天数
        /// </summary>   
        ///<param name="DateStr1">时间1</param>
        /// <param name="DateStr2">时间2</param>
        /// <param name="p_strFileName"></param>
        /// <returns></returns>
        public long GetDayToDayCount(DateTime DateStr1, DateTime DateStr2, string p_strFileName)
        {
            long lDateStr1 = GetDayCount(DateStr1, p_strFileName);
            long lDateStr2 = GetDayCount(DateStr2, p_strFileName);
            return lDateStr1 - lDateStr2;
        ...
        /// <param name="p_strFileName"></param>
        /// <returns></returns>
        public long GetDayToNowCount(DateTime DateStr, string p_strFileName)
        {
            long AllTime = GetAllLineCount(p_strFileName);
            long RTime = GetDayCount(DateStr, p_strFileName);
            return AllTime - RTime;
        ...
        /// <param name="p_strFileName"></param>
        /// <returns></returns>
        public long GetDayCount(DateTime DateStr, string p_strFileName)
        {
            //获取日期下的偏移地址
            int StartPostion = GetDayPostion(DateStr, p_strFileName);
            long All = GetAllLineCount(p_strFileName);
            //K线40为一个长度周期
            long CountValue = 0;
        ...
        /// <param name="p_strFileName"></param>
        /// <returns></returns>
        public long GetAllLineCount(string p_strFileName)
        {
            //K线40为一个长度周期
            long CountValue = 0;
            FileStream stream = new FileStream(p_strFileName, FileMode.Open, FileAccess.Read, FileShare.ReadWrite);
            CountValue = stream.Length / 40;
            stream.Close();
            return CountValue;
        }
              /// <summary>
        ...
        public long GetAllLineCount()
        {
            //K线40为一个长度周期
            long CountValue = 0;
            FileStream stream = new FileStream(StockCode, FileMode.Open, FileAccess.Read, FileShare.ReadWrite);
            CountValue = stream.Length / 40;
            stream.Close();
            return CountValue;
        }
        ...
        /// 获取日期到日期
        /// </summary>
        /// <param name="DL1">位置1</param>
        /// <param name="DL2">位置2</param>
        /// <param name="p_strFileName">股票文件</param>
        /// <returns></returns>
        public int GetAreaLineToNow(int DL1, int DL2, string p_strFileName)
        {
            DayStockInfo dsi = new DayStockInfo();
            int MyDateTime = 0;
            //开始位置
            int StartPostion = 0;
            //结束位置
            ...
            while (stream.CanRead && stream.Position < stream.Length)
            {
                ...
                dsi.day = day;
                dsi.OpenPrice = b_reader.ReadInt32();
                dsi.HighestPrice = b_reader.ReadInt32();
                dsi.LowestPrice = b_reader.ReadInt32();
                dsi.ClosePrice = b_reader.ReadInt32();
                dsi.Amount = b_reader.ReadInt32();
                dsi.TransCount = b_reader.ReadInt32();
                dsi.Padding1 = b_reader.ReadInt32();
                dsi.Padding2 = b_reader.ReadInt32();
                dsi.Padding3 = b_reader.ReadInt32();
                proc++;
                PicLineDrawValue(dsi, proc);

                CountValue++;
                if (CountValue > EndPostion)//到结束位置，跳出循环
                {
                    PicLineDrawValueOver(dsi);
                    return CountValue;
                }
            }
            PicLineDrawValueOver(dsi);
            //Console.WriteLine("一共条数:" + CountValue.ToString());
            return CountValue;
        }
        /// <summary>
        /// 获取日期到日期
        ...
        /// <param name="p_strFileName">股票文件</param>
        /// <returns></returns>
        public int GetAreaLineToNow(int DL1, int DL2)
        {
            DayStockInfo dsi = new DayStockInfo();
            int MyDateTime = 0;
            //开始位置
        ...
        /// <param name="p_strFileName">股票文件</param>
        /// <returns></returns>
        public int GetAreaLineToNow(DateTime DateStr1, DateTime DateStr2, string p_strFileName)
        {
            DayStockInfo dsi = new DayStockInfo();
            int MyDateTime = 0;
            //开始位置
            ...
            //结束位置
            int EndPostion = 0;
            int DL1 = GetDayPostion(DateStr1, p_strFileName);
            int DL2 = GetDayPostion(DateStr2, p_strFileName);
            if (DL1 > DL2)//判断，大的为结束位置，小的为开始位置
            {
                StartPostion = DL2;
                EndPostion = DL1;
        ...
        /// <param name="p_strFileName">股票文件</param>
        /// <returns></returns>
        public int GetAreaLineToNow(DateTime DateStr1, DateTime DateStr2)
        {
            DayStockInfo dsi = new DayStockInfo();
            int MyDateTime = 0;
            //开始位置
            ...
            //结束位置
            int EndPostion = 0;
            int DL1 = GetDayPostion(DateStr1, stockCode);
            int DL2 = GetDayPostion(DateStr2, stockCode);
            if (DL1 > DL2)//判断，大的为结束位置，小的为开始位置
            {
                StartPostion = DL2;
                EndPostion = DL1;
        ...
        /// <param name="p_strFileName">股票文件</param>
        /// <returns></returns>
        public int GetAreaLineToNow(DateTime DateStr, string p_strFileName)
        {
            DayStockInfo dsi = new DayStockInfo();
            int MyDateTime = 0;
            //获取日期下的偏移地址
            ...
            while (stream.CanRead && stream.Position < stream.Length)
            {
                ...
                proc++;
                PicLineDrawValue(dsi, proc);
                //Console.WriteLine("时间:{0}\t开盘:{1}\t最高:{2}\t最低:{3}\t收盘:{4}\t金额:{5}\t总量:{6} - \t振幅:{7}\t-:{8}\t-:{9}",
                //day.ToShortDateString(), nOpenPrice / 1000, nHighestPrice / 1000, nLowestPrice / 1000, nClosePrice / 1000, nAmount, nTransCount,
        ...
        /// <param name="p_strFileName">股票文件</param>
        /// <returns></returns>
        public int GetAreaLineToNow(DateTime DateStr)
        {
            DayStockInfo dsi = new DayStockInfo();
            int MyDateTime = 0;
            //获取日期下的偏移地址
            int StartPostion = GetDayPostion(DateStr, stockCode);
            int CountValue = 0;
            FileStream stream = new FileStream(stockCode, FileMode.Open, FileAccess.Read, FileShare.ReadWrite);
            BinaryReader b_reader = new BinaryReader(stream);
            stream.Position = (long)StartPostion * 40;
        ...
        /// </summary>
        /// <param name="p_strFileName"></param>
        public void GetLine(string p_strFileName)
        {
            int MyDateTime = 0;
            FileStream stream = new FileStream(p_strFileName, FileMode.Open, FileAccess.Read, FileShare.ReadWrite);
            BinaryReader b_reader = new BinaryReader(stream);
        ...
        /// 获取前几日相应的指标值
        /// </summary>
        /// <param name="ColumeName">字段公式或名称</param>
        /// <param name="counday">天数,1为昨日</param>
        /// <param name="DTime">日期</param>
        /// <returns></returns>
        public double StockValueREF(string ColumeName, int counday, DateTime DateStr, string p_strFileName)
        {
            double myvalue = 0;
            DayStockInfo dsi = new DayStockInfo();
            int MyDateTime = 0;
            //获取日期下的偏移地址     
            int EndPostion = GetDayPostion(DateStr, p_strFileName);
            int StartPostion = EndPostion - counday;
            if (StartPostion < 0)//如果开始位置为负，证明没有那么多数，直接返回0
            {
                myvalue = 0;
            }
            FileStream stream = new FileStream(p_strFileName, FileMode.Open, FileAccess.Read, FileShare.ReadWrite);
            ...
            while (stream.CanRead && stream.Position < stream.Length)
            {
                ...
                dsi.Padding2 = b_reader.ReadInt32();
                dsi.Padding3 = b_reader.ReadInt32();
                string Rtvalue = GetLineTechValue(ColumeName, dsi);
                myvalue = double.Parse(Rtvalue);
                break;
        ...
        /// <param name="DTime">日期</param>
        /// <returns></returns>
        public double StockValueREF(string ColumeName, int counday, DateTime DateStr)
        {
            double myvalue = 0;
            DayStockInfo dsi = new DayStockInfo();
            int MyDateTime = 0;
            //获取日期下的偏移地址     
            int EndPostion = GetDayPostion(DateStr, stockCode);
            int StartPostion = EndPostion - counday;
            if (StartPostion < 0)//如果开始位置为负，证明没有那么多数，直接返回0
            {
                myvalue = 0;
        ...
        /// <param name="DTime">日期</param>
        /// <returns></returns>
        public double StockValueMA(string ColumeName, int average, DateTime DateStr, string p_strFileName)
        {
            DayStockInfo dsi = new DayStockInfo();
            int MyDateTime = 0;
            //获取日期下的偏移地址
            int EndPostion = GetDayPostion(DateStr, p_strFileName);
            int StartPostion = EndPostion - average;
            ...
            while (stream.CanRead && stream.Position < stream.Length)
            {
                ...
                dsi.Padding2 = b_reader.ReadInt32();
                dsi.Padding3 = b_reader.ReadInt32();
                AllCount += double.Parse(GetLineTechValue(ColumeName, dsi));
                CountValue++;
            }
            double Rvalue = AllCount / (double)average;
            return Rvalue;
        }
        /// <summary>
        /// 求平均数
        ...
        /// <param name="DTime">日期</param>
        /// <returns></returns>
        public double StockValueMA(string ColumeName, int average, DateTime DateStr)
        {
            DayStockInfo dsi = new DayStockInfo();
            int MyDateTime = 0;
            //获取日期下的偏移地址
        ...
        /// <returns></returns>
        public string StockCompute(string strResult)
        {
            string TmpResult = "";   //结果
            try
            {
      
                TmpResult = this.dtAllMsg.Compute(strResult, "").ToString();
            }
            catch
            {
        ...
        /// 获取日线值
        /// </summary>
        /// <param name="ValueName">日线属性名</param>
        /// <param name="DSI">值</param>
        /// <returns></returns>
        public int GetLineValue(string ValueName, DayStockInfo DSI)
        {
            //CLOSE 收盘
            //OPEN  开盘
            //VOL   量
            ...
            {
                case "CLOSE":
                    RtValue = DSI.ClosePrice;
                    break;
                case "OPEN":
                    RtValue = DSI.OpenPrice;
                    break;
                case "VOL":
                    RtValue = DSI.TransCount;
                    break;
                case "AMOUNT":
                    RtValue = DSI.Amount;
                    break;
                case "HIGH":
                    RtValue = DSI.HighestPrice;
                    break;
                case "LOW":
                    RtValue = DSI.LowestPrice;
                    break;
        ...
        /// <returns></returns>
        public string GetLineTechValue(string ValueName, DayStockInfo DSI)
        {
            ValueName = ValueName.Replace("CLOSE", GetLineValue("CLOSE", DSI).ToString());
            ValueName = ValueName.Replace("OPEN", GetLineValue("OPEN", DSI).ToString());
            ValueName = ValueName.Replace("VOL", GetLineValue("VOL", DSI).ToString());
            ValueName = ValueName.Replace("AMOUNT", GetLineValue("AMOUNT", DSI).ToString());
            ValueName = ValueName.Replace("HIGH", GetLineValue("HIGH", DSI).ToString());
            ValueName = ValueName.Replace("LOW", GetLineValue("LOW", DSI).ToString());
            string RtValue = StockCompute(ValueName);
            return RtValue;
        ...
        private void PicGraph_PreviewKeyDown(object sender, PreviewKeyDownEventArgs e)
        {
            ...
            if (processBarValue == 0)
            {
                ...
                bool locateCrossHairFlag = false;
                    ...
                    {
                        SetCursorPos(this.Left, this.Top);//把鼠标移动到软件上方，防止十字线乱动
                        currentScrollStep = 1;
                        flag = true;
                        if (showCrossHair)
                        {
                   ...
                   if(e.KeyData== Keys.Up)
                   {
                    ...
                    {
                        SetCursorPos(this.Left, this.Top);//把鼠标移动到软件上方，防止十字线乱动
                        currentScrollStep = LastVisibleRecord - FirstVisibleRecord;
                        flag = true;
                        if (showCrossHair)
                        {
                            CrossHairScrollLeft();
        ...
        /// <param name="sender"></param>
        /// <param name="e"></param>
        public void ChartGraph_KeyDown(object sender, KeyEventArgs e)
        {
        ...
        public void CrossHairScrollLeft()
        {
            ...
            this.crossOverIndex = currentRecord - currentScrollStep;
            if (crossOverIndex < 0)
            {
                crossOverIndex = 0;
            }
        ...
        public void CrossHairScrollRight()
        {
            ...
            {
                if (crossOverIndex >= maxRecord - currentScrollStep)
                {
                    crossOverIndex = maxRecord - currentScrollStep;
                }
            }
            if (currentRecord >= this.LastVisibleRecord - currentScrollStep)
            {
                ScrollRight(currentScrollStep);
            }
        }
        ...
        /// </summary>
        /// <returns></returns>
        public CandleSeries AddVolCandle(string candleName, string openfield, string highfield, string lowfield, string closefield, int panelID, bool displayTitleField)
        {
        ...
        {
            this.AddExponentialMovingAverage(buyEMA, buyEMA, 2, buyTarget, panelID);
            this.SetTrendLineStyle(buyEMA, Color.SkyBlue, Color.Red, 1, DashStyle.Solid);
            this.AddExponentialMovingAverage(sellEMA, sellEMA, 50, sellTarget, panelID);
